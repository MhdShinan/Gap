...
router.get("/getAcademicDetails/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    // Find the specific academic detail by ID
    const academic = user.academicDetails.find(
      (academicDetail) => academicDetail._id.toString() === req.params.id
    );

    if (academic) {
      // Check if the endDate is a string (like "Present") or a valid Date object
      const formattedAcademic = {
        ...academic._doc,
        startDate: formatDate(academic.startDate), // Format the start date
        endDate: typeof academic.endDate === "string" 
          ? academic.endDate // If it's a string (like "Present"), use as-is
          : formatDate(academic.endDate), // Otherwise, format as a date
      };

      // Send back the found academic detail with the formatted dates
      res.json({ academic: formattedAcademic });
    } else {
...
router.post("/academicDetails", isAuthenticated, async (req, res) => {
  ...
  const user = await User.findById(req.user.id);
  const validEndDate = endDate && !isNaN(new Date(endDate).getTime()) ? new Date(endDate) : "Present";
...
// Edit Academic Details Endpoint
      ...
      { _id: req.user.id },
      {
        $push: {
          professionalDetails: {
            ...
            companyName,
            locationType,
            startDate: new Date(startDate), // Ensure startDate is valid
            endDate: validEndDate, // Use "Present" if endDate is invalid
            skills,
          },
        },
      }
...
// Edit Professional Details Endpoint
router.post(
